...
5. 先用单个、带总截止时间的轻量命令验证登录；不要在传输参数未知时并行启动多个 SSH。
6. 若 TCP 已建立但握手停在 `expecting SSH2_MSG_KEX_ECDH_REPLY`，立即尝试 `KexAlgorithms=curve25519-sha256`；成功后在本次任务的所有 SSH/SCP/rsync 中复用该参数。
7. 验证成功后启用短时连接复用，并把负载、磁盘、内存、系统版本等只读检查合并到一个 SSH 会话。
8. 先只读盘点，再执行变更、备份、迁移或服务操作；每阶段记录命令、目的、预期和实际结果。
9. 验证服务状态、数据完整性、权限、空间和可回滚性。
10. 默认保留访问状态；只有用户明确要求时，才删除远端精确公钥行和本地密钥对。
...
```bash
timeout 12s ssh -i "$key_path" -p "$port" \
...
失败时不要重复运行相同命令，也不要立即判断为公钥授权失败。只跑一次 12 秒详细诊断，并按最后阶段分类：
...
if have timeout; then deadline='timeout 12s'; elif have gtimeout; then deadline='gtimeout 12s'; else deadline=''; fi
